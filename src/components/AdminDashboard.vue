    ...
    <div id="elementInfo">
      <div v-if="selectedElement && selectedElement.isEdge">
        ...
        <select v-model="selectedElement.add_hours.type" name="edgeHoursType">
          <option
            v-for="addedHourType in addedHourTypes"
            :key="addedHourType"
            :value="addedHourType.value"
            :selected="addedHourType.value==selectedElement.add_hours.type"
          >{{ addedHourType.type }}</option>
        </select>
        <br />
      ...
      <div v-else-if="selectedElement">
        ...
        <textarea v-model="selectedElement.id" name="nodeId"></textarea>
        <br />
      ...
      <button type="button" @click="removeElement">Remove element</button>
    </div>
  </div>
</template>
<script>
    ...
    parseJson() {
      ...
      for (const node of Object.keys(this.nodes)) {
        for (const option of this.nodes[node].options) {
          try {
            let node_color = this.cy.$id(option.next_node).style()[
              "background-color"
            ];
